...
export default function Cart({
  ...
  const cartSummary = cart.reduce<Record<string, FruitSummary>>(
    (acc, fruit) => {
  ...
  return (
    <div className="h-full">
      <div className="mt-4 font-bold text-center">Total: £{totalPrice}</div>
        ...
        {Object.values(cartSummary).map((fruit, index) => (
          <li key={index} className="flex justify-between items-center mb-2">
            <span className="font-bold">
              {fruit.emoji} x {fruit.quantity}
            </span>
            <span>
              £
              {(
                fruit.quantity * cart.find((f) => f.name === fruit.name)!.price
              ).toFixed(2)}
            </span>
          </li>
        ))}
